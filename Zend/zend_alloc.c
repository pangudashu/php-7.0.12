...
#endif
{
	...
	while (1) {
		if (UNEXPECTED(chunk->free_pages < pages_count)) {
			...
			while (1) {
				/* skip allocated blocks */
				while (tmp == (zend_mm_bitset)-1) { //-1表示当前chunk所有page都已分配 11111111 11111111 11111111 11111111
					i += ZEND_MM_BITSET_LEN;
					if (i == ZEND_MM_PAGES) {
						if (best > 0) {
							page_num = best;
				...
				tmp &= tmp + 1;
				/* skip free blocks */
				while (tmp == 0) { //如果当前tmp表示的64/32个page前面连续分配了则tmp = 0,这步操作可以快速跳过这些已分配page 
					i += ZEND_MM_BITSET_LEN;
				...
				/* set bits from 0 to "bit" */ //请当前分组所有已分配page的上一页也设为已分配
				tmp |= tmp - 1;
			}
		}
...
ZEND_API zend_mm_heap *zend_mm_startup(void) //useless
{
	return zend_mm_init();
}
